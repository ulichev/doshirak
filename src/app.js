...
// Используются корни слов (без окончаний) — один корень покрывает все словоформы.
// Порядок важен: первое совпадение побеждает, более специфичные — выше.
const CAT_KEYWORDS = [
  ...
  { keywords: [
    ...
    'окей','o\'кей','билла','billa','азбука вкус','азбука вкуса',
    'фреш','рынок','базар','фермер','продукт','супермарк','гастроном',
    // Доставка еды
    'самокат','яндекс.еда','яндекс еда','delivery club','деливери',
    'сбермаркет','купер','мегамарк','vprok','впрок','igooods',
    // Фастфуд
    'макдоналд','макдак','mcdonald','бургер кинг','burger king','kfc','кфс',
    'subway','субвей','шаурм','шавух','фалафел','хот-дог','хотдог',
    'фастфуд','fast food','пирожк','чебурек','беляш','блин','блинн',
    // Продукты питания
    'хлеб','булочн','батон','молок','кефир','творог','сыр ','яйц',
    'мясо','рыба','овощ','фрукт','крупа','макарон','сахар','масл ',
    'groceries','food','продовольств',
    // Общие
    'еда','продукт','питани','шашлык','пельмен','вареник',
  ], cat: 'food' },

  // ── КАФЕ И РЕСТОРАНЫ ───────────────────────────────────────────────────────
  { keywords: [
    // Типы заведений
    'кафе','ресторан','бистро','столов','буфет','кофейн','бар ','паб',
    'таверн','трактир','харчевн',
    // Напитки
    'кофе','coffee','капучин','латте','эспрессо','американо','раф',
    'старбакс','starbucks','cofix','кофикс','даблби','double b',
    'шоколадниц','кофеман','coffee bean',
    // Кухни и блюда
    'пицц','pizza','суши','sushi','роллы','rolls','wok','вок','рамен','ramen',
    'бургер ','бургерн','burger ','стейк','паста','лапш','димсам',
    'тайск','итальянск','японск','китайск','грузинск','армянск','корейск',
    // Сети
    'додо','dodo pizza','папа джонс','papa john','ваби саби','якитория',
    'теремок','крошка картошка','мишлен','il primo',
    // Приёмы пищи
    'обед','ужин','завтрак','бранч','lunch','dinner','breakfast',
  ], cat: 'cafe' },
  ...
  { keywords: [
    ...
    'мцд','мцк','монорельс','фурникулёр',
    // Такси и каршеринг
    'такси','taxi','uber','убер','яндекс.такси','яндекс такси','ситимобил',
    'gett','гетт','везёт','везет','максим такси',
    'каршер','ситидрайв','делимобил','яндекс драйв','rentacar',
    // Дальние поездки
    'электричк','ласточк','сапсан','аэроэкспресс','ржд','rzd','поезд',
    'автовокзал','автобусный','автостанц',
    'авиа','самолёт','самолет','аэрофлот','победа','s7','utair','ютэйр',
    // Личный транспорт
    'бензин','дизель','заправк','лукойл','роснефть','газпромнефть',
    'парковк','платн парк','автомойк','техосмотр','шиномонтаж',
    // Общее
    'билет','транспорт',
  ], cat: 'transport' },
  ...
  { keywords: [
    ...
    'сбермегамаркет','мегамарк','aliexpress','алиэкспресс','joom','авито',
    'юла','lamoda','ламода','kupivip','брендс',
    // Одежда и обувь
    'зара','zara','h&m','hm ','uniqlo','юникло','befree','befree',
    'gloria jeans','глория джинс','colin\'s','твое ','твоё ',
    'adidas','адидас','nike','найк','puma','пума','reebok','рибок',
    'обувь','кроссовк','кеды','ботинк','сапог','туфл','босоножк',
    // Электроника
    'мвидео','m.video','эльдорадо','dns ','re:store','apple store',
    'samsung','iphone','ноутбук','планшет','наушник','телефон',
    // Дом и строительство
    'икеа','ikea','леруа','leroy','obi ','castorama','хофф','hoff',
    'интерьер','мебель','диван','шкаф','светильник',
    // Красота и уход
    'рив гош','rive gauche','иль де ботэ','l\'etoile','золотое яблок',
    'косметик','парфюм','духи','крем','шампун','уход',
    // Спорт
    'спортмастер','декатлон','decathlon','спортивн',
    // Общее
    'покупк','покупки','товар','магазин',
  ], cat: 'shopping' },

  // ── ЗДОРОВЬЕ ───────────────────────────────────────────────────────────────
  { keywords: [
    // Аптеки
    'аптек','apteka','планет здоровь','горздрав','ригла','имплозия',
    // Медицина
    'врач','доктор','поликлин','клиник','больниц','госпитал','медцентр',
    'стоматолог','зубн','ортодонт','окулист','офтальмолог',
    'гинеколог','уролог','кардиолог','невролог','педиатр',
    'скорая','хирург','операц',
    // Диагностика
    'анализ','узи','мрт','кт ','рентген','флюорографи','экг',
    'инвитро','helix','хеликс','гемотест','ситилаб','лаборатор',
    // Препараты и уход
    'лекарств','таблетк','капсул','витамин','биодобавк','бад ',
    'капли','мазь','пластырь','бинт','шприц',
    // Прочее
    'медицин','здоровь','аллергия','прививк','вакцин','массаж','физиотерап',
  ], cat: 'health' },

  // ── РАЗВЛЕЧЕНИЯ ────────────────────────────────────────────────────────────
  { keywords: [
    // Стриминг и подписки
    'netflix','нетфликс','okko','окко','кинопоиск','иви ','start ','more.tv',
    'spotify','спотифай','яндекс музык','apple music','vk музык',
    'youtube premium','twitch','steam','стим','playstation','xbox',
    'nintendo','epic games','app store','google play','подписк',
    // Игры
    'игр','геймер','game','gaming',
    // Кино и театр
    'кино','кинотеатр','синема','imax','cinema','4dx',
    'театр','филармони','опер','балет','спектакл',
    'концерт','фестивал',
    // Активный отдых
    'боулинг','каток','батут','картинг','пейнтбол','лазертаг',
    'квест','escape room','страйкбол',
    // Ночная жизнь
    'клуб ','ночной клуб','кальян','hookah','лаунж','lounge',
    'бар ','паб ','коктейл',
    // Культура
    'музей','выставк','галере','зоопарк','аквапарк','аттракцион',
    'парк развлечен','луна-парк',
    // Общее
    'развлечен','досуг','отдых',
  ], cat: 'fun' },

  // ── ДОХОДЫ ─────────────────────────────────────────────────────────────────
  { keywords: ['зарплат','получк','аванс','оклад','выплат','премия','бонус'], cat: 'salary' },
  { keywords: ['фриланс','подработк','проект','гонорар','заказ','клиент оплат','вознагражд'], cat: 'freelance' },
  { keywords: ['подарок','день рождения','подаро','сувенир','букет','цветы'], cat: 'gift' },
  { keywords: ['долг','вернул','возврат','отдал','перевод от','скинул'], cat: 'debt_ret' },
];
